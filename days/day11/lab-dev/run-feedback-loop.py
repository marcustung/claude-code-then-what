...
def main():
    ...
    ap.add_argument("--withhold", default="",
                    help="comma-separated paths removed from the work copy before the loop; "
                         "used to simulate \"只有紅燈，沒有規格\" and to test the failure branch")
    ap.add_argument("--model", default="sonnet",
                    help="driver model; the loop only shows its value when round 1 fails")
    a = ap.parse_args()
    ...
    withheld = [w.strip() for w in a.withhold.split(",") if w.strip()]
    for w in withheld:
        t = work / w
        if t.exists():
            t.unlink()
    ...
    report = {"case": "teaching rule change", "model": a.model, "withheld": withheld, "max_rounds": a.max_rounds, "baseline_exit": code,
    ...
    for i in range(1, a.max_rounds + 1):
        ...
        head = ("Read CLAUDE.md, tests/DomainTests/Program.cs and src/Domain/Cancellation.cs. "
                "The written specification is NOT available to you; infer the required behaviour from the failing test only."
                if withheld else
                "Read CLAUDE.md, plan.md, specs/rules-v2.md, tests/DomainTests/Program.cs and src/Domain/Cancellation.cs.")
        prompt = head + """

Implement ONLY the Domain rule change. Keep type signatures and shipped-order behavior. No real payments, API changes or test changes.
...
Latest actual runner output:\n""" + feedback
        ...
        args = [cli, "-p", "--model", a.model, "--effort", "medium", "--setting-sources", "project",
