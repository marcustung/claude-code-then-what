...
$dir=Join-Path $PSScriptRoot 'diagram'; $p=[IO.File]::ReadAllText((Join-Path $dir 'prompt.txt'),$utf8)
...
@{name='diagram';started_utc=$start.ToString('o');ended_utc=[DateTime]::UtcNow.ToString('o');exit_code=$proc.ExitCode;arguments=$args}|ConvertTo-Json|Set-Content (Join-Path $out 'meta.json') -Encoding UTF8
"diagram exit=$($proc.ExitCode)"
