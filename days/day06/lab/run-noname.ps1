$ErrorActionPreference="Stop"; [Console]::InputEncoding=[Text.UTF8Encoding]::new($false); $utf8=[Text.UTF8Encoding]::new($false); $cli="$env:USERPROFILE\.local\bin\claude.exe"
$dir=Join-Path $PSScriptRoot 'noname'; $p=[IO.File]::ReadAllText((Join-Path $dir 'prompt.txt'),$utf8)
...
@{name='noname';started_utc=$start.ToString('o');ended_utc=[DateTime]::UtcNow.ToString('o');exit_code=$proc.ExitCode;arguments=$args}|ConvertTo-Json|Set-Content (Join-Path $out 'meta.json') -Encoding UTF8
"noname exit=$($proc.ExitCode)"
